# HG6145F1 Password Generator (AT Nightmares)

Android GUI application for generating **admin** and **Wi-Fi passwords**
for **FiberHome HG6145F1 routers (Algérie Télécom)** using the router
MAC address.

This tool is designed for recovery and educational use.

## Features

-   Admin password generation\
-   Wi-Fi password generation\
-   Simple and fast Android GUI\
-   Offline operation\
-   No Python or PC required

## Requirements

-   Android phone\
-   APK installation enabled\
-   Disable **Auto Blocker / Play Protect** before installation (app is
    not from Play Store)

## How to Use

1.  Install the APK on your phone\
2.  Disable **Auto Blocker / Play Protect** if prompted\
3.  Open the app\
4.  Tap the **Gen** button\
5.  Paste the router **MAC address** when requested\
6.  The app instantly generates:
    -   Admin password\
    -   Wi-Fi password

## Releases
Version 1.0
The first public release of the app.
Simple, lightweight, and effective.
Focused on one goal: generating admin and Wi-Fi passwords quickly and reliably.

Version 2.0
A major upgrade with a refined experience.
Faster, smoother, and more powerful than ever.
Redesigned interface for better usability.
Now includes Dark Mode and Light Mode for comfortable use in any environment.
Improved stability and overall performance.

## Supported Device

-   FiberHome **HG6145F1**\
-   Algérie Télécom firmware variants

## Disclaimer

This application is provided **for educational and recovery purposes
only**.\
Use it **only on routers you own or have explicit authorization to
access**.\
The author is not responsible for misuse or illegal activity.

## License

MIT License. See the `LICENSE` file for details.
